...
<h<?php echo $headerLevel; ?> class="js_heading"><span class="backh"> <span
	...
	title="<?php echo JText::_('TPL_PHOCA_COLOR_CLICK'); ?>"
	onclick="auf('module_<?php echo $module->id; ?>'); return false"
	class="opencloselink" id="link_<?php echo $module->id?>"> <span
	class="no"><img src="templates/phoca_color/images/plus.png"
	alt="<?php if ($state == 1) { echo JText::_('TPL_PHOCA_COLOR_ALTOPEN');} else {echo JText::_('TPL_PHOCA_COLOR_ALTCLOSE');} ?>" />
...
function modChrome_phocaTabs($module, $params, $attribs)
{
	...
	if ($modulecount == 1) {
		...
		foreach($modules as $rendermodule) {
			...
			{
			echo '<a href="#" class="unseen" onclick="nexttab(\'module_'. $rendermodule->id.'\');return false;" id="next_'.$rendermodule->id.'">'.JText::_('TPL_PHOCA_COLOR_NEXTTAB').'</a>';
			}
			echo '</div>';
		}
		$modulecount--;
